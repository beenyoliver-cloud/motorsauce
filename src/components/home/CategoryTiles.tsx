      ...
      {tiles.map((t) => {
        ...
        return (
          <Link
            key={t.label}
            href={t.href}
            className="group rounded-xl border border-gray-200 bg-white p-3 sm:p-4 flex flex-col items-center text-center hover:shadow-sm hover:border-gray-300 transition"
          >
            <div className="rounded-lg bg-gray-50 border border-gray-200 p-2 mb-2">
              <Icon size={20} className="text-gray-700" />
            </div>
            <div className="text-xs sm:text-sm font-semibold text-gray-900 group-hover:text-black">{t.label}</div>
          </Link>
        );
      })}
    </div>
