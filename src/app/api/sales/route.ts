...
export async function GET(req: NextRequest) {
  try {
    ...
    const formattedSales = (sales || []).map((item: any) => ({
      ...
      shipping_carrier: item.order.shipping_carrier ?? null,
      tracking_number: item.order.tracking_number ?? null,
      delivery_confirmed_at: item.order.delivery_confirmed_at ?? null,
      payout_status: item.order.payout_status ?? null,
